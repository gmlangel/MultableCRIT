...
function MasterPage(){
    ...
    this.addEvents = function(){
        ...
        tabContainer.addEventListener('postchange',function(e){
            ...
            ////设置title
            //var titNode = ons._util.createElement("<div id='mainT'>"+dataProvider[selecteIdx].titName+"</div>")
            //var titBar = document.getElementById("mainTitle");
            ////移除旧的
    ...
    this.createMod = function(title,modName){
        new ModuleFactoryProxy().loadModule(modName,function(path){
            var tabNode = ons._util.createElement('<ons-tab label="'+title+'" page="'+path+'" '+(currentLoadCount == 0 ? 'active':'')+'></ons-tab>');
            var tabContainer  = document.getElementById('mainTabbar');
